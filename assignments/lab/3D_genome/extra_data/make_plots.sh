...
if [ -d ${BOWTIE2_FINAL_OUTPUT_DIR} ]; then
	...
	then
	    ...
	    nb=$(find $MAPPING_DIR  -name "*.mapstat" | wc -l | awk '{print $1}')
            if [[ $nb > 0 ]]; then
	...
	then
	    ...
	    nb=$(find $MAPPING_DIR  -name "*.pairstat" | wc -l | awk '{print $1}')
            if [[ $nb > 0 ]]; then
...
if [[ -d ${DATA_DIR} ]]; then
	...
	## be sure that the proc_hic step was run 
	    nb=$(find $DATA_DIR  -name "*.RSstat" | wc -l | awk '{print $1}')
            if [[ $nb > 0 ]]; then
	...
	## be sure that the merge_valid_interaction step was run
	    if [[ -d ${STATS_DIR} ]]; then
		nb=$(find $STATS_DIR  -name "*.mergestat" | wc -l | awk '{print $1}')
		if [[ $nb > 0 ]]; then
